...
func init() {
	...
	registerHash("tiger160,3", func() Hash { return newTiger(20, 3) })
	registerHash("tiger192,3", func() Hash { return newTiger(24, 3) })
	registerHash("tiger128,4", func() Hash { return newTiger(16, 4) })
	registerHash("tiger160,4", func() Hash { return newTiger(20, 4) })
	registerHash("tiger192,4", func() Hash { return newTiger(24, 4) })
}
...
func (d *tigerDigest) Reset() {
	d.s[0] = 0x0123456789ABCDEF
	d.s[1] = 0xFEDCBA9876543210
	d.s[2] = 0xF096A5B4C3B2E187
	d.bufLen = 0
	d.len = 0
}
...
func (d *tigerDigest) Write(p []byte) (int, error) {
	...
	if d.bufLen > 0 {
		...
		d.bufLen += fill
		p = p[fill:]
		if d.bufLen == tigerBlockSize {
			tigerCompress(&d.s, d.buf[:], d.passes)
			d.bufLen = 0
		}
	}

	for len(p) >= tigerBlockSize {
		tigerCompress(&d.s, p[:tigerBlockSize], d.passes)
		p = p[tigerBlockSize:]
	}
...
func (d *tigerDigest) Sum(in []byte) []byte {
	...
	// Padding: 0x01 byte, then zeros, then 64-bit LE length in bits.
	var tmp [72]byte
	tmp[0] = 0x01
	bufLen := uint64(d0.bufLen)
	var padLen uint64
	if bufLen < 56 {
		padLen = 56 - bufLen
	} else {
	...
	var digest [24]byte
	binary.LittleEndian.PutUint64(digest[0:], d0.s[0])
	binary.LittleEndian.PutUint64(digest[8:], d0.s[1])
	binary.LittleEndian.PutUint64(digest[16:], d0.s[2])
	return append(in, digest[:d0.size]...)
}

func tigerRound(a, b, c *uint64, x uint64, mul uint64) {
	*c ^= x
	*a -= tigerT1[byte(*c)] ^ tigerT2[byte(*c>>16)] ^ tigerT3[byte(*c>>32)] ^ tigerT4[byte(*c>>48)]
	*b += tigerT4[byte(*c>>8)] ^ tigerT3[byte(*c>>24)] ^ tigerT2[byte(*c>>40)] ^ tigerT1[byte(*c>>56)]
	*b *= mul
}
...
// Tiger S-boxes from the reference implementation
